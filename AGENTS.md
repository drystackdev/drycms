...
- `src/apps/pages/**` (gitignored, NOT hand-edited) - a build-time-only
  ...
  automatically; `bun run build:worker`/`deploy` currently do NOT (a
  `wrangler r2 object list` incompatibility blocks the `--remote` pull as of
  2026-08-09 - see that script's own doc comment), so run `bun run build`
  or `bun run pages:sync --pull [--remote]` by hand first if `src/apps/pages`
  might be stale/missing before building/deploying the worker. Still what
  Vite bundles into the Cloudflare Worker and what a VEI session in PROD
  renders through (dev never touches it anymore) - safe to delete locally at
  any time (nothing hand-authored lives there), just don't `build:worker`/
  `deploy` right after without repopulating it first.
