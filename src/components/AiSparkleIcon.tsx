 ...
 * chose directly, not sourced from Iconify (checked both `solar` and
 * `lucide`, the only two sets `scripts/build-icons.mjs` draws from - no
 * match), so it's hand-written here instead of forced through that
 * generated pipeline. Same `IconProps` shape as `icons.tsx`'s generated
 * icons so it's a drop-in alongside them.
 */
