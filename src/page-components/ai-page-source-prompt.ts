...
export function buildPageSourceSystemPrompt({ lang, path, currentSource, projectFiles, projectContext }: BuildPageSourceSystemPromptParams): string {
  ...
  return [
    ...
    '- Every line is either `key: |` followed by indented raw lines (a block literal - use this for EVERY value: chat text, a summary, a file\'s full code), or `key: value` on one line (a plain scalar - use this ONLY for `path` on a `kind: code`/`kind: read` reply, or `root` on a `kind: read` reply).',
    "- Indent consistently with exactly 2 spaces per level. Never use tabs.",
    "",
    "There are three possible top-level replies. A single request may chain SEVERAL of them in a row (e.g. `kind: code` for one file, then `kind: code` again for a second file that imports it) - only `kind: chat` ends the exchange, so after a `code` or `read` reply you immediately get another turn to continue or wrap up:",
    "",
    ...
    "path: component/Card.tsx",
    "```",
    '   `root: source` reads another file in THIS project\'s own `pages/`, `component/`, `styles/`, or `md/` folders (e.g. a path you see imported via `@component/Card`, which resolves to `component/Card.tsx`, or another context file linked from the project notes above) - use it when you need to see a component before importing it, how a sibling page/component is written, or a linked context file, to match this project\'s own conventions. `root: docs` reads one of this project\'s own documentation files listed above by its exact path (e.g. `docs/ARCHITECTURE.md`).',
    "",
