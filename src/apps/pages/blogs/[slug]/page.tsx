...
export default async function BlogDetailPage({ params }: { params: { slug: string } }) {
  ...
  if (!post) {
    return (
      <div class="mx-auto max-w-3xl px-4 py-24 text-center">
        ...
        <a href="/blogs" class="mt-6 inline-block text-sm font-medium text-teal-700 hover:underline">
          ← Quay lại Blog
        </a>
      </div>
    );
  ...
  return (
    <article class="mx-auto max-w-3xl px-4 py-16">
      ...
      <div class="mt-4">
        <span class="rounded-full bg-teal-100 px-4 py-1 text-sm font-medium text-teal-700">{post.tag}</span>
      ...
      <div class="mt-12 flex items-center gap-4 rounded-2xl border border-slate-200 p-5">
        ...
        <a href="/about" class="ml-auto shrink-0 text-sm font-medium text-teal-700 hover:underline">
          Xem thêm →
        </a>
      </div>

      <div class="mt-8 flex flex-col items-center gap-3 rounded-2xl bg-teal-600 px-8 py-10 text-center text-white">
          ...
          <div class="mt-4 grid gap-6 sm:grid-cols-3">
            {relatedPosts.map((related) => (
              <a
                ...
                class="block overflow-hidden rounded-2xl border border-slate-200 hover:border-teal-300"
              >
                <div class="h-32 bg-slate-200" />
                <div class="space-y-1 p-4">
                  <span class="text-xs font-semibold uppercase tracking-wide text-teal-700">{related.tag}</span>
