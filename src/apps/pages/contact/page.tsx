...
const CHANNEL_ICONS: Record<string, ComponentChildren> = {
  phone: (
    <path
...
export default async function ContactPage() {
  const contact = await dry().singleton("contact").get();
  if (!contact) return null;
  const { header, channels } = contact;

  return (
    <div class="mx-auto max-w-6xl px-4 py-16">
      <div class="max-w-2xl">
        <span class="rounded-full bg-red-100 px-4 py-1 text-sm font-medium text-red-900">{header.eyebrow}</span>
        <h1 class="mt-4 text-3xl font-bold text-slate-900 sm:text-4xl">{header.headline}</h1>
        <p class="mt-3 text-sm leading-relaxed text-slate-600">{header.description}</p>
      </div>
