export default async function AboutPage() {
  ...
  const { intro, story, missionSection, missionItems, experienceSection, experienceItems, pressSection, pressMentions, bottomCta } =
    about;
    ...
    <div>
      <section class="mx-auto max-w-6xl px-4 py-16">
        <div class="grid items-center gap-10 sm:grid-cols-2">
          ...
          />
          <div class="space-y-4">
            <span class="rounded-full bg-red-100 px-4 py-1 text-sm font-medium text-red-900">{intro.eyebrow}</span>
            <h1 class="text-3xl mt-2 font-bold text-slate-900 sm:text-4xl">{intro.headline}</h1>
            <p class="text-sm leading-relaxed text-slate-600">{intro.description}</p>
          </div>
        </div>
      </section>
      ...
      <section class="mx-auto max-w-6xl px-4 py-16">
        <h2 class="text-2xl font-bold text-slate-900">{missionSection.heading}</h2>
        <ul class="mt-6 space-y-3">
          {missionItems.map((item) => (
            <li key={item.text} class="flex items-start gap-3 text-sm leading-relaxed text-slate-600">
      ...
      <section class="bg-slate-50">
        <div class="mx-auto max-w-6xl px-4 py-16">
          <h2 class="text-2xl font-bold text-slate-900">{experienceSection.heading}</h2>
      ...
      <section class="mx-auto max-w-6xl px-4 py-16">
        ...
        <p class="mt-2 max-w-xl text-sm leading-relaxed text-slate-600">{pressSection.description}</p>
      ...
      <section class="mx-auto max-w-6xl px-4 py-16">
        <div class="flex flex-col items-center gap-4 rounded-2xl bg-red-800 px-8 py-12 text-center text-white">
          ...
          <p class="max-w-xl text-sm leading-relaxed text-red-50">{bottomCta.description}</p>
          <a href={bottomCta.ctaHref ?? "/contact"} class="rounded-full bg-white px-6 py-3 text-sm font-semibold text-red-900 hover:bg-red-50">
            {bottomCta.ctaLabel}
          </a>
        </div>
      </section>
