      ...
      {open ? (
        <div class="absolute inset-x-0 top-16 z-20 border-b border-slate-200 bg-white px-4 py-3 shadow-sm">
          <nav class="flex flex-col gap-1">
            {NAV_LINKS.map((link) => (
              <a key={link.href} href={link.href} class="rounded-md px-3 py-2 text-sm font-medium text-slate-700 hover:bg-red-50 hover:text-red-900">
                {link.label}
              </a>
            ))}
          </nav>
    ...
    <div class="flex min-h-screen flex-col bg-white font-sans text-slate-900">
      <header class="sticky top-0 z-10 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div class="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
          <a href="/" class="flex items-center gap-2 text-lg font-bold text-red-900">
            <span class="flex h-9 w-9 items-center justify-center rounded-full bg-red-800 text-white">+</span>
            [Tên thương hiệu]
          </a>
          <nav class="hidden gap-6 sm:flex">
            {NAV_LINKS.map((link) => (
              <a key={link.href} href={link.href} class="text-sm font-medium text-slate-600 hover:text-red-900">
                {link.label}
              </a>
            ))}
          </nav>
      ...
      <footer class="border-t border-slate-200 bg-slate-50">
        <div class="mx-auto grid max-w-6xl gap-8 px-4 py-12 sm:grid-cols-3">
          <div>
            <p class="text-lg font-bold text-red-900">[Tên thương hiệu]</p>
