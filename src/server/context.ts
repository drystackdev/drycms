...
/**
 * The one context shape every route handler under `server/routes/` receives.
 * `env` carries the Cloudflare-adapter-specific bag the D1 content engine
 * reads its binding from - the Node adapter passes `{}` (D1 is never
 * selectable outside a Workers-shaped runtime), the Workers adapter passes
 * the real `env` argument from `fetch(request, env, ctx)` straight through.
 */
