...
export const GET: DryRouteHandler = async (context) => {
  try {
    ...
    return jsonResponse({ hasAnyUser: anyUser, user: context.session });
  } catch (error) {
...
export const POST: DryRouteHandler = async (context) => {
  try {
    ...
    if (endpoint === "register-first-admin") {
      ...
      return withSessionCookie(jsonResponse({ user: sessionUser }, 201), context, token);
    }
    ...
    if (endpoint === "login") {
      ...
      const token = await signSession(sessionUser);
      return withSessionCookie(jsonResponse({ user: sessionUser }), context, token);
    }
