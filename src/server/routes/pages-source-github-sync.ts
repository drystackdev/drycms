 ...
 * `dry-http` (`handler.ts`'s `SYSTEM_BUILD_RESOURCE_ID` check) - this is
...
export const POST: DryRouteHandler = async (context) => {
  ...
  if (!type) return jsonResponse({ pushed: false, reason: "not-configured" });

  const row = await entries.getSingletonEntry(type, allTypes);
  const value = (row?.value ?? {}) as GithubSyncSingletonValue;
  if (!row || !value.enabled || !value.repo || !value.branch) {
    return jsonResponse({ pushed: false, reason: "not-configured" });
  }

  const raw = await entries.getRawEntry(type, row.id);
  const encryptedToken = typeof raw?.token === "string" ? raw.token : "";
  if (!encryptedToken) return jsonResponse({ pushed: false, reason: "not-configured" });

  let token: string;
  try {
    token = await decryptSecret(encryptedToken);
  } catch {
    return jsonResponse({ pushed: false, reason: "The stored GitHub token cannot be decrypted with the current DRYCMS_SECRET_KEY." });
  }
  ...
  const result = await pushPagesSourceSnapshot(sourceByPath, { repo: value.repo, branch: value.branch, token }, message);
