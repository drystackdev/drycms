import type { EntryValue, MaskedValue } from "./entry-codec.js";
 ...
 * `undefined` fallback. Framework-agnostic (no browser dependency), so it's
 * shared between the client (`ContentEntryEditor.tsx`'s new-entry form, via
 * `pages/content-entry-editor/blank-value.ts`'s re-export) and the server
 * (a singleton's auto-created first row, see `entries-sqlite.ts`'s
