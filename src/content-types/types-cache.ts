 ...
 *
 * ALSO writes the real on-disk `src/apps/dry.generated.d.ts` (still needed
 * by local `tsc`/IDE tooling - `reader.md`'s own decision, unchanged) - but
...
export async function writeGeneratedDryTypes(output: string): Promise<void> {
  ...
  try {
    ...
    const generatedDir = resolvePath(process.cwd(), "src/apps");
