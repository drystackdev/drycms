...
async function createTestCollection(page: Page): Promise<{ id: string; title: string }> {
...
test.describe("Content Type editor", () => {
  ...
  test("Add Field dialog: 2-column layout, type-gated right column, text validation interactions, number step default", async ({
    page,
  }) => {
    await page.goto("/dry/content-types/new/collection");
    ...
    const columns = await dialog.locator(".field-dialog-grid").evaluate(
      (el) => getComputedStyle(el).gridTemplateColumns.split(" ").length,
    );
    expect(columns).toBe(2);

    // Right column empty until a type is chosen.
    await expect(dialog.getByText("Display")).toHaveCount(0);
    await expect(dialog.getByText("Validation")).toHaveCount(0);
    ...
    // Pick "text" - regex/format/minLength appear, and are mutually exclusive.
  ...
  test("Remove field shows a confirm dialog before removing", async ({ page }) => {
    ...
    try {
      await page.getByRole("button", { name: "+ Add Field" }).click();
      const dialog = page.getByRole("dialog");
      await dialog.getByLabel("Label", { exact: true }).fill("Removable");
      await dialog.getByRole("button", { name: "Select…" }).click();
      await page.getByRole("option", { name: "Text" }).click();
