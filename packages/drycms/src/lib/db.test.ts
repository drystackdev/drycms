...
describe("createDB", () => {
  it("validates required/unique/format on creates() and hashes+verifies PASSWORD on the row", async () => {
    ...
    const missingName = await user.creates({ email: "a@b.com" });
    expect(missingName.error?.name).toBeTruthy();

    const badEmail = await user.creates({ name: "A", email: "not-an-email" });
    ...
    const dup = await user.creates({ name: "Khan2", email: "khan@gmail.com" });
    expect(dup.error?.email).toBeTruthy();

    // verifyPassword nằm trên chính row trả về, không cần truyền lại id/table.
    expect(await row!.verifyPassword("secret123")).toBe(true);
    expect(await row!.verifyPassword("wrong")).toBe(false);
  });
  ...
  it("supports where operators, sort tie-breaker, and pagination", async () => {
    ...
    const gmailUsers = await user.where({ email: { endsWith: "gmail.com" } }).get();
    expect(gmailUsers.map((u) => u.name).sort()).toEqual(["A", "C"]);

    const page = await user.sort({ name: "asc" }).pagination(2, 0).get();
    expect(page).toHaveLength(2);
    expect(page[0]?.name).toBe("A");

    const uppercased = await user
      .where({ name: "A" })
  ...
  it("REF: filters via EXISTS in where and includes nested object in select", async () => {
    ...
    let user = db.table("user", { name: db.TEXT().REQUIRED() });
    user = user.addColumn({ role: db.REF(role) });

    const { statements } = await migrateInTempDir(db);
    for (const stmt of statements) raw.exec(stmt);

    const adminRole = (await role.creates({ name: "Admin" })).success;
    await user.creates({ name: "Khan", role: adminRole!.id });
    await user.creates({ name: "Guest" });

    // `q` bên trong callback được suy type theo đúng cột của bảng `role`.
    const filtered = await user.where({ role: (q) => q.where({ name: "Admin" }) }).get();
    expect(filtered).toHaveLength(1);
    expect(filtered[0]?.name).toBe("Khan");

    const withInclude = await user
      .where({ name: "Khan" })
  ...
  it("REFS: join-table filter (some) and array include", async () => {
    ...
    let user = db.table("user", { name: db.TEXT().REQUIRED() });
    user = user.addColumn({ permissions: db.REFS(permission) });

    const { statements } = await migrateInTempDir(db);
    for (const stmt of statements) raw.exec(stmt);

    const read = (await permission.creates({ name: "read" })).success;
    const write = (await permission.creates({ name: "write" })).success;
    const khan = (await user.creates({ name: "Khan" })).success;
    await user.creates({ name: "Guest" });
    ...
    const filtered = await user.where({ permissions: (q) => q.where({ name: "write" }) }).get();
    expect(filtered).toHaveLength(1);
    expect(filtered[0]?.name).toBe("Khan");
  });
  ...
  it("migrate() writes an incremental ALTER TABLE when a column is added", async () => {
    ...
    user = user.addColumn({ nickname: db.TEXT() });
  ...
  it("update() and delete() apply to rows matched by where()", async () => {
    ...
    const created = (await user.creates({ name: "Khan" })).success;

    const updated = await user.where("id", created!.id).update({ name: "Khan Tran" });
    expect(updated[0]?.name).toBe("Khan Tran");

    const deleted = await user.where("id", created!.id).delete();
    expect(deleted).toHaveLength(1);

    const remaining = await user.where({ id: created!.id }).get();
