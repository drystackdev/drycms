...
import { VIRTUAL_CONFIG_TYPES, dryFixOptimizeDeps, dryVirtualConfig } from './virtual.js';
	...
	return {
		...
		hooks: {
			'astro:config:setup': (params) => {
				...
				// duplicate renderer.
				const hasPreact = config.integrations.some((i) => i.name === PREACT_INTEGRATION_NAME);
				if (!hasPreact) {
