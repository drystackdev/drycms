...
function context(endpoint: string, query: Record<string, string> = {}): APIContext {
  const url = new URL(`http://localhost/dry/api/iconify/${endpoint}`);
  for (const [key, value] of Object.entries(query)) url.searchParams.set(key, value);
  return { params: { slug: endpoint }, request: new Request(url), url } as unknown as APIContext;
}
