...
function groupLabelFor(id: string): string | undefined {
	return groups.find((group) => group.items.some((item) => item.id === id))?.label;
}
...
export default function Showcase({ tab }: Props) {
	...
	return (
		<div class="main" ref={main.ref} style="padding: 1rem; padding-top: 0;">
			...
			<div class="showcase">
				<aside class="showcase-nav" aria-label="Showcase sections" ref={nav.ref}>
					<nav>
						{groups.map((group) => (
				...
				<div class="showcase-body">
					...
					<nav class="pager" aria-label="Showcase pagination">
						{prevId ? (
							...
							<button type="button" class="outline pager-btn" disabled style="text-align: left;">
								<Icon name="ArrowLeft" />
								<span class="pager-text">
									<small>Previous</small>
									<strong></strong>
						...
						{nextId ? (
							<a role="button" class="pager-btn pager-btn-next" href={`${path}/showcase/${nextId}`}>
								<span class="pager-text">
									<small>Next</small>
...
function TabsDemoPreview() {
	...
	return (
		<div class="stack" style="width: 100%" data-tabs ref={ref}>
			<div role="tablist">
				<button type="button" role="tab" aria-selected="true" aria-controls="sc-panel-1">
					Overview
				</button>
				<button type="button" role="tab" aria-selected="false" aria-controls="sc-panel-2">
					Settings
				</button>
				<button type="button" role="tab" aria-selected="false" aria-controls="sc-panel-3">
					History
				</button>
			</div>
	...
	switch (id) {
							...
							<tbody>
								{tableRows.slice(0, 4).map((row) => (
									<tr key={row.name}>
										...
										<td>
											<span class={`badge ${statusVariant(row.status)}`}>{row.status}</span>
										</td>
		...
		case 'skeleton':
			return (
				...
				>
					<div class="row">
						<span class="skeleton" style="height: 3rem; width: 3rem; border-radius: 50%"></span>
		...
		case 'tabs':
			return (
				<Demo id="tabs" title="Tabs" description="Driven by role and aria-selected." code={code.tabs!}>
					<TabsDemoPreview />
				</Demo>
			);
		...
		case 'tooltip':
			return (
				<Demo
					...
					code={code.tooltip!}
				>
					<button type="button" class="outline" data-tooltip="Saved 2 minutes ago">
						Hover me
					</button>
			...
			return (
				...
				>
					...
					<dialog id="sc-dialog">
						...
						<footer>
							<button type="button" class="outline" command="close" commandfor="sc-dialog">
								Cancel
							</button>
							<button type="button" class="destructive" command="close" commandfor="sc-dialog">
								Delete
							</button>
						</footer>
					</dialog>
		...
		case 'datatable':
			return (
				<Demo id="datatable" title="DataTable" description="Sort, filter and paginate." code={code.datatable!}>
					<div style="width: 100%">
			...
			return (
				...
				>
					<div style="width: 100%; max-width: 20rem">
						<Select options={collectionOptions} defaultValue="docs" name="collection" />
					</div>
				</Demo>
			);
		...
		case 'combobox':
			return (
				...
				>
					<div style="width: 100%; max-width: 20rem">
						<Combobox options={collectionOptions} defaultValue="changelog" name="collection" />
					</div>
				</Demo>
			);
		...
		case 'islands':
			return (
				...
				>
					<p class="muted">
						The theme toggle and the mobile sidebar toggle are in the topbar above. The theme
						toggle cycles system → light → dark and stores the choice in localStorage.
					</p>
				</Demo>
			);
